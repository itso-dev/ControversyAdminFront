...
<!--          <td class="py-3"><input type="checkbox" :checked="item.chk" @click="item.chk = !item.chk"></td>-->
    ...
    <div class="m-auto w text-center flex justify-center gap-3 text-lg mt-10">
      <a class="bg-gray-200 px-3 py-1 cursor-pointer" v-if="this.contents_pages.start !== 1" @click="onChangePage(this.contents_pages.start - 1)">
        <i class="fas fa-chevron-left"></i>
      </a>
      <a class="bg-gray-400 px-3 py-1 cursor-pointer" :class="this.contents_pages.page == page ? 'font-bold' : ''" v-for="page in this.contents_pages.pagesList"
         @click="onChangePage(page)">
        {{ page }}
      </a>
      <a class="bg-gray-200 px-3 py-1 cursor-pointer" v-if="this.contents_pages.end !== this.contents_pages.end_page + 1"
